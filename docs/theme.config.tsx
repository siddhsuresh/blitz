...
export default {
  ...
  head() {
    ...
    if (asPath === "/") {
      ...
      ogUrl = `${frontMatter.ogImage}`
    } else {
      const type = asPath.startsWith("/docs/blitz-auth")
        ? "Blitz Auth"
        : asPath.startsWith("/docs/blitz-rpc")
        ? "Blitz RPC"
        : ""
      const title = frontMatter.title ? `&title=${encodeURIComponent(frontMatter.title)}` : ""

      ogUrl = `/api/og?title=${title}`
    }
    ...
    return (
      <>
        ...
        <link rel="icon" type="image/png" sizes="128x128" href="/128x128-Favicon-Purple.png"></link>
        <link rel="icon" type="image/png" sizes="32x32" href="/32x32-Favicon-Purple.png"></link>
        <link rel="icon" type="image/png" sizes="16x16" href="/16x16-Favicon-Purple.png"></link>
        <link rel="manifest" href="/manifest.json"></link>
        <link rel="mask-icon" href="/safari-pinned-tab.svg" color="#5600C2"></link>
        ...
        <meta
          name="twitter:image"
          content={OG_IMAGE_URL + frontMatter.title ? frontMatter.title : "Get Started"}
        />
        <meta
          name="og:title"
  ...
  logo() {
    return (
      <svg viewBox="0 0 165 66" className="blitz-logo">
        ...
        <path className="blitz-logo-color" d="M0.241243 33.2639H10.9742C15.0585 33.2639 18.9054 35.1835 21.3612 38.4471L31.9483 52.5165C32.1484 52.7824 32.1786 53.1393 32.026 53.435L25.9232 65.2592C25.6304 65.8265 24.8455 65.8932 24.4612 65.3835L0.241243 33.2639Z" />
        <path className="blitz-logo-color" d="M42.4727 33.2822H31.7398C27.6555 33.2822 23.8086 31.3626 21.3528 28.0991L10.7656 14.0297C10.5656 13.7638 10.5354 13.4068 10.688 13.1111L16.7908 1.28696C17.0836 0.719654 17.8684 0.652924 18.2528 1.16266L42.4727 33.2822Z" />
      </svg>
    )
  },
  banner: {
